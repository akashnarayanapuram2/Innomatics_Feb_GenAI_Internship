...
class BulkOrder(BaseModel):
    ...
    itmes:          List[BulkOrderRequest] = Field(..., min_items=1)

@app.post('/orders/bulk')
def place_bulk_order(order: BulkOrder):
    confirmed, failed, grand_total = [], [], 0
    for item in order.itmes:
    ...
    return {"company": order.customer_name, "confirmed": confirmed, "failed": failed, "grand_total": grand_total}
